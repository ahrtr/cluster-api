...
func TestEvaluateDefragRule(t *testing.T) {
	...
	const (
		dbSize      = 1_500_000_000.0
		dbSizeInUse = 900_000_000.0
		dbQuota     = 2_000_000_000.0
	)
	...
	tests := []struct {
		...
		},
		{
			...
			rule:        "dbSizeFree > 500000000.0 && dbQuotaUsage > 0.7",
			dbSize:      dbSize,
			dbSizeInUse: dbSizeInUse,
			dbQuota:     dbQuota,
			wantResult:  true,
		},
		{
			...
			rule:        "dbSizeFree > 500000000.0 && dbQuotaUsage > 0.8",
			dbSize:      dbSize,
			dbSizeInUse: dbSizeInUse,
			dbQuota:     dbQuota,
			wantResult:  false,
